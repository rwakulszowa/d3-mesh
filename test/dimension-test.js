...
tape("Dimension fromFunction computes and normalizes a function", function(test) {
  var dim = mesh.dimension.fromFunction(function(x) { return 10 * x; }, 2);
  test.same(dim.expand(), [0, 0.5, 1]);
  test.end();
});

tape("Dimension fromNodes sets and normalizes", function(test) {
  var dim = mesh.dimension.fromNodes([0, 1, 4]);
  test.same(dim.expand(), [0, 0.25, 1]);
  test.end();
});

tape("Dimension fromSizes computes and normalizes", function(test) {
